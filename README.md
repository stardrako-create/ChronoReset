...
- **Genomic instability → cellular senescence, upgraded to `/2`**: FoxM1 is repressed with age, and that repression drives mitotic mis-segregation and aneuploidy-driven senescence; re-inducing FOXM1 in aged/progeroid fibroblasts prevents the aneuploidy and reverses senescence phenotypes (Ribeiro, Macedo et al. 2018, *Nat Commun*). This mechanism was specifically checked because an earlier version of this model flagged an unverified mTOR→FOXM1 speculation — that link didn't hold up, but this genomic-stability mechanism does, with real mammalian lifespan data behind it: cyclic FOXM1 transgene induction significantly extended lifespan in both Hutchinson-Gilford progeria *and* naturally aged mice (Ribeiro, Macedo et al. 2022, *Nature Aging*). Two independent causal mechanisms — DDR-driven senescence entry and FOXM1/aneuploidy — now converge on this one edge, which is why the tier moved up, not because either mechanism alone got stronger.
...
- ~~FOXM1's relationship to this model is unverified~~ — verified, and the original speculation was wrong. No good evidence turned up for an mTOR→FOXM1 link; what does exist is a *different*, better-evidenced mechanism now encoded in `intervene_genomic_instability`: FoxM1 repression with age drives aneuploidy-driven senescence, and re-inducing it prevents this (Ribeiro, Macedo et al. 2018, *Nat Commun*) — with real mammalian lifespan data behind it (cyclic FOXM1 transgene induction significantly extended lifespan in both Hutchinson-Gilford progeria *and* naturally aged mice, Ribeiro, Macedo et al. 2022, *Nature Aging*). This upgraded the `genomic_instability → cellular_senescence` coupling from `/4` to `/2`: two independent causal mechanisms now converge on the same edge.
